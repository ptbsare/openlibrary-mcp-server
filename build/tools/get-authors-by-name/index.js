...
const handleGetAuthorsByName = async (args, axiosInstance) => {
    const parseResult = GetAuthorsByNameArgsSchema.safeParse(args);
    if (!parseResult.success) {
        const errorMessages = parseResult.error.issues
            .map((e) => `${e.path.join(".")}: ${e.message}`)
            .join(", ");
        throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for get_authors_by_name: ${errorMessages}`);
    }
    const authorName = parseResult.data.name;
    try {
        const response = await axiosInstance.get("/search/authors.json", // Use the author search endpoint
        {
        ...
        const authorResults = response.data.docs.map((doc) => ({
            key: doc.key,
            name: doc.name,
