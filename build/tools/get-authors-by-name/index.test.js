...
describe("handleGetAuthorsByName", () => {
    ...
    it("should return author information when authors are found", async () => {
        ...
        expect(mockedAxiosInstanceGet).toHaveBeenCalledWith("/search/authors.json", { params: { q: "Tolkien" } });
    ...
    it("should return a message when no authors are found", async () => {
        ...
        mockedAxiosInstanceGet.mockResolvedValue(mockAxiosResponse);
        const result = await handleGetAuthorsByName(mockArgs, mockAxiosInstance);
        expect(mockedAxiosInstanceGet).toHaveBeenCalledWith("/search/authors.json", { params: { q: "NonExistentAuthor" } });
        expect(result.isError).toBeUndefined();
        expect(result.content).toEqual([
            {
                type: "text",
    ...
    it("should handle Axios errors with response", async () => {
        ...
        const axiosError = new AxiosError("Request failed with status code 500", "ERR_BAD_RESPONSE", mockConfig, null, mockResponse);
        mockedAxiosInstanceGet.mockRejectedValue(axiosError);
        const result = await handleGetAuthorsByName(mockArgs, mockAxiosInstance);
        expect(mockedAxiosInstanceGet).toHaveBeenCalledWith("/search/authors.json", { params: { q: "ErrorCase" } });
    ...
    it("should handle Axios errors without response (e.g., network error)", async () => {
        ...
        const mockConfig = createMockConfig();
        const axiosError = new AxiosError("Network Error", // Message
        "ECONNREFUSED", // Code
        mockConfig, // Config
        null, // Request
        undefined);
        mockedAxiosInstanceGet.mockRejectedValue(axiosError);
        const result = await handleGetAuthorsByName(mockArgs, mockAxiosInstance);
        expect(mockedAxiosInstanceGet).toHaveBeenCalledWith("/search/authors.json", { params: { q: "NetworkErrorCase" } });
        expect(result.isError).toBe(true);
        expect(result.content).toEqual([
            {
                type: "text",
    ...
    it("should handle generic errors", async () => {
        ...
        const result = await handleGetAuthorsByName(mockArgs, mockAxiosInstance);
        expect(mockedAxiosInstanceGet).toHaveBeenCalledWith("/search/authors.json", { params: { q: "GenericError" } });
        expect(result.isError).toBe(true);
        expect(result.content).toEqual([
            {
                type: "text",
    ...
    it("should throw McpError for invalid arguments (empty name)", async () => {
        const mockArgs = { name: "" };
        await expect(handleGetAuthorsByName(mockArgs, mockAxiosInstance)).rejects.toThrow(new McpError(ErrorCode.InvalidParams, "Invalid arguments for get_authors_by_name: name: Author name cannot be empty"));
        expect(mockedAxiosInstanceGet).not.toHaveBeenCalled();
    });
    it("should throw McpError for invalid arguments (missing name)", async () => {
        const mockArgs = {};
        await expect(handleGetAuthorsByName(mockArgs, mockAxiosInstance)).rejects.toThrow(new McpError(ErrorCode.InvalidParams, "Invalid arguments for get_authors_by_name: name: Invalid input: expected string, received undefined"));
        expect(mockedAxiosInstanceGet).not.toHaveBeenCalled();
    });
});
