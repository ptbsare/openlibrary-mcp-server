...
describe("handleGetAuthorInfo", () => {
    it("should return author info for a valid author key", async () => {
        ...
        const result = await handleGetAuthorInfo({ author_key: "OL26320A" }, mockAxiosInstance);
        expect(mockAxiosInstance.get).toHaveBeenCalledWith("/authors/OL26320A.json");
    ...
    it("should handle bio as an object", async () => {
        ...
        const result = await handleGetAuthorInfo({ author_key: "OL27346A" }, mockAxiosInstance);
        expect(result.isError).toBeUndefined();
        expect(result.content).toEqual([
            {
                type: "text",
    ...
    it("should throw McpError for invalid author key format", async () => {
        await expect(handleGetAuthorInfo({ author_key: "invalid-key" }, mockAxiosInstance)).rejects.toThrow(new McpError(ErrorCode.InvalidParams, "Invalid arguments for get_author_info: author_key: Author key must be in the format OL<number>A"));
    });
    it("should throw McpError for empty author key", async () => {
        await expect(handleGetAuthorInfo({ author_key: "" }, mockAxiosInstance)).rejects.toThrow(new McpError(ErrorCode.InvalidParams, "Invalid arguments for get_author_info: author_key: Author key cannot be empty, author_key: Author key must be in the format OL<number>A"));
    });
    it("should return an error message for a 404 Not Found response", async () => {
        const authorKey = "OL00000A";
        const axiosError = new AxiosError(`Request failed with status code 404`, "404", undefined, undefined, {
            status: 404,
        ...
        const result = await handleGetAuthorInfo({ author_key: authorKey }, mockAxiosInstance);
    ...
    it("should return a message if API returns 200 but no data", async () => {
        ...
        mockAxiosInstance.get.mockResolvedValue({ data: null }); // Simulate no data
        const result = await handleGetAuthorInfo({ author_key: authorKey }, mockAxiosInstance);
        expect(result.isError).toBeUndefined();
        expect(result.content).toEqual([
            {
