...
import { describe, it, expect, beforeEach, vi } from "vitest"; // Use vitest imports
...
describe("handleGetBookById", () => {
    ...
    it("should return book details when given a valid OLID", async () => {
        ...
        const mockApiResponse = {
            records: {
                "/books/OL7353617M": {
                    recordURL: "https://openlibrary.org/books/OL7353617M/The_Lord_of_the_Rings",
                    data: {
                    ...
                    details: {
                        info_url: "https://openlibrary.org/books/OL7353617M/The_Lord_of_the_Rings",
        ...
        const result = await handleGetBookById(mockArgs, mockAxiosInstance); // Cast to any for simplicity
        expect(mockAxiosInstance.get).toHaveBeenCalledWith("/api/volumes/brief/olid/OL7353617M.json");
        ...
        expect(parsedResult).toHaveProperty("open_library_edition_key", "/books/OL7353617M");
        expect(parsedResult).toHaveProperty("open_library_work_key", "/works/OL45804W");
        expect(parsedResult).toHaveProperty("cover_url", "https://covers.openlibrary.org/b/id/8264411-M.jpg");
        expect(parsedResult).toHaveProperty("info_url", "https://openlibrary.org/books/OL7353617M/The_Lord_of_the_Rings");
        expect(parsedResult).toHaveProperty("preview_url", "https://archive.org/details/lordofrings00tolk_1");
    });
    it("should return book details when given a valid ISBN", async () => {
        ...
        mockAxiosInstance.get.mockResolvedValue({ data: mockApiResponse });
        const result = await handleGetBookById(mockArgs, mockAxiosInstance);
        expect(mockAxiosInstance.get).toHaveBeenCalledWith("/api/volumes/brief/isbn/9780547928227.json");
        expect(result).toEqual({
            content: [
                {
                    type: "text",
    ...
    it("should throw McpError for invalid arguments", async () => {
        ...
        await expect(handleGetBookById(invalidArgs, mockAxiosInstance)).rejects.toThrow(McpError);
        try {
            await handleGetBookById(invalidArgs, mockAxiosInstance);
        }
        catch (error) {
            expect(error).toBeInstanceOf(McpError);
            expect(error.code).toBe(ErrorCode.InvalidParams);
            expect(error.message).toContain("Invalid arguments for get_book_by_id");
            expect(error.message).toContain("idType must be one of: isbn, lccn, oclc, olid");
        }
    ...
    it('should return "No book found" message when API returns empty records', async () => {
        ...
        mockAxiosInstance.get.mockResolvedValue({ data: mockApiResponse });
        const result = await handleGetBookById(mockArgs, mockAxiosInstance);
        expect(mockAxiosInstance.get).toHaveBeenCalledWith("/api/volumes/brief/olid/OL_NONEXISTENT.json");
        expect(result).toEqual({
            content: [
                {
                    type: "text",
    ...
    it('should return "No book found" message on 404 API error', async () => {
        ...
        const result = await handleGetBookById(mockArgs, mockAxiosInstance);
        expect(mockAxiosInstance.get).toHaveBeenCalledWith("/api/volumes/brief/isbn/0000000000.json");
        expect(result).toEqual({
            content: [
                {
                    type: "text",
    ...
    it("should return generic API error message for non-404 errors", async () => {
        ...
        mockAxiosInstance.get.mockRejectedValue(axiosError);
        const result = await handleGetBookById(mockArgs, mockAxiosInstance);
        expect(mockAxiosInstance.get).toHaveBeenCalledWith("/api/volumes/brief/olid/OL1M.json");
        expect(result).toEqual({
            content: [
                {
                    type: "text",
