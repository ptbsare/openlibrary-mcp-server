...
export const handleGetBookById = async (args, axiosInstance) => {
    const parseResult = GetBookByIdArgsSchema.safeParse(args);
    if (!parseResult.success) {
        const errorMessages = parseResult.error.issues
            .map((e) => `${e.path.join(".")}: ${e.message}`)
            .join(", ");
        throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for get_book_by_id: ${errorMessages}`);
    }
    ...
    try {
        ...
        const bookDetails = {
            ...
            number_of_pages: recordData.number_of_pages ?? recordDetails?.number_of_pages,
            ...
            preview_url: record.details?.preview_url ?? recordData.ebooks?.[0]?.preview_url,
        };
        // Clean up undefined fields
        Object.keys(bookDetails).forEach((key) => {
            const typedKey = key;
            if (bookDetails[typedKey] === undefined ||
