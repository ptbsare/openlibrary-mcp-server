...
describe("OpenLibraryServer", () => {
    ...
    beforeEach(() => {
        ...
        mockMcpServer = Server.mock.results[0].value;
    ...
    describe("get_book_by_title tool", () => {
        it("should correctly list the get_book_by_title tool", async () => {
            ...
            const listToolsHandler = mockMcpServer.setRequestHandler.mock.calls.find((call) => call[0] === ListToolsRequestSchema)?.[1];
            expect(listToolsHandler).toBeDefined();
            if (listToolsHandler) {
                const result = await listToolsHandler({}); // Call the handler
    ...
    describe("get_authors_by_name tool", () => {
        it("should correctly list the get_authors_by_name tool", async () => {
            ...
            if (listToolsHandler) {
                ...
                expect(result.tools).toHaveLength(6);
                const authorTool = result.tools.find((tool) => tool.name === "get_authors_by_name");
        ...
        it("should handle CallTool request for get_authors_by_name successfully", async () => {
            const callToolHandler = mockMcpServer.setRequestHandler.mock.calls.find((call) => call[0] === CallToolRequestSchema)?.[1];
    ...
    describe("get_author_info tool", () => {
        it("should correctly list the get_author_info tool", async () => {
            ...
            if (listToolsHandler) {
                const result = await listToolsHandler({});
                expect(result.tools).toHaveLength(6);
                const authorInfoTool = result.tools.find((tool) => tool.name === "get_author_info");
                ...
                expect(authorInfoTool.inputSchema).toEqual({
                    ...
                    properties: {
                        author_key: {
                            type: "string",
                            description: "The Open Library key for the author (e.g., OL23919A).",
                        },
                    },
                    required: ["author_key"],
                });
        ...
        it("should handle CallTool request for get_author_info successfully", async () => {
            ...
            if (callToolHandler) {
                ...
                expect(result.content).toHaveLength(1);
                expect(result.content[0].type).toBe("text");
                expect(JSON.parse(result.content[0].text)).toEqual(mockApiResponse.data);
            }
        });
    });
    describe("get_author_photo tool", () => {
        it("should correctly list the get_author_photo tool", async () => {
            ...
            if (listToolsHandler) {
                const result = await listToolsHandler({});
                expect(result.tools).toHaveLength(6);
                const authorPhotoTool = result.tools.find((tool) => tool.name === "get_author_photo");
        ...
        it("should handle CallTool request for get_author_photo successfully", async () => {
            ...
            if (callToolHandler) {
                ...
                expect(result.content).toHaveLength(1);
                expect(result.content[0].type).toBe("text");
                expect(result.content[0].text).toBe("https://covers.openlibrary.org/a/olid/OL23919A-L.jpg");
            }
        });
    });
    describe("get_book_cover tool", () => {
        it("should correctly list the get_book_cover tool", async () => {
            ...
            if (listToolsHandler) {
                ...
                const bookCoverTool = result.tools.find((tool) => tool.name === "get_book_cover");
    ...
    it("should handle CallTool request for an unknown tool", async () => {
        ...
        if (callToolHandler) {
            ...
            await expect(callToolHandler(mockRequest)).rejects.toThrow(new McpError(ErrorCode.MethodNotFound, "Unknown tool: unknown_tool"));
