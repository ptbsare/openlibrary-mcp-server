...
You can run this MCP server directly from the git repository using npx:

```bash
npx github:ptbsare/openlibrary-mcp-server
```

Or with the full git URL:

```bash
npx git+https://github.com/ptbsare/openlibrary-mcp-server.git
```
...
```json
{
  "mcpServers": {
    "open-library": {
      "command": "npx",
      "args": ["github:ptbsare/openlibrary-mcp-server"]
    }
  }
}
```
